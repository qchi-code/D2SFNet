...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #####noise
        idx = lbl.sum(axis=(1, 2)) > 0
        positive_indices = np.where(idx)[0]
        if len(positive_indices) >= 4:
            ...
            print('bugou5ge')
        idx_mask = np.zeros_like(idx)
        idx_mask[selected_indices] = True
        sample['image'] = torch.from_numpy(img[idx_mask])
        sample['label'] = torch.from_numpy(lbl[idx_mask])
    ...
    def __getitem__(self, pat_idx):
        ...
        while size < self.min_size:
            ...
            if not len(subsets):
                ...
                return self.__getitem__(np.random.randint(low=0, high=len(self.image_dirs)))
...
class TrainDataset_source(Dataset):
    ...
    def __init__(self, args):
        ...
        self.min_size = args['min_size']
        self.test_label = args['test_label']
        self.exclude_label = args['exclude_label_source']
        self.use_gt = args['use_gt']

        # if args['dataset'] == 'CMR':
        # self.image_dirs = glob.glob(os.path.join(args['sources_data_path'], 'cmr_MR_normalized/image*'))
        # self.label_dirs = glob.glob(os.path.join(args['sources_data_path'], 'cmr_MR_normalized/label*'))
        # self.image_dirs = glob.glob(os.path.join(args['sources_data_path'], 'chaos_MR_T2_normalized/image*'))
        # self.label_dirs = glob.glob(os.path.join(args['sources_data_path'], 'chaos_MR_T2_normalized/label*'))
        self.image_dirs = glob.glob(os.path.join(args['sources_data_path'], 'sabs_CT_normalized/image*'))
        self.label_dirs = glob.glob(os.path.join(args['sources_data_path'], 'sabs_CT_normalized/label*'))
        # self.image_dirs = glob.glob(os.path.join(args['sources_data_path'], 'mms_MR_normalized/image*'))
        # self.label_dirs = glob.glob(os.path.join(args['sources_data_path'], 'mms_MR_normalized/label*'))
        #
        self.image_dirs = sorted(self.image_dirs, key=lambda x: int(x.split('_')[-1].split('.nii.gz')[0]))
        self.label_dirs = sorted(self.label_dirs, key=lambda x: int(x.split('_')[-1].split('.nii.gz')[0]))
        # self.sprvxl_dirs = glob.glob(os.path.join(args['data_dir'], 'supervoxels_' + str(args['n_sv']), 'super*'))
